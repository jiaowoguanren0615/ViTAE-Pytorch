    ...
    def __init__(self, img_size=224, in_chans=3, embed_dims=64, token_dims=64, downsample_ratios=4, kernel_size=7,
        ...
        if tokens_type not in ['none', 'embedding']:
            self.PCM = nn.Sequential(
                ...
                nn.Conv2d(embed_dims, embed_dims, kernel_size=(3, 3), stride=PCMStride[1], padding=(1, 1),
                          groups=group),  # the 1st convolution
                nn.BatchNorm2d(embed_dims),
                nn.SiLU(inplace=True),
                nn.Conv2d(embed_dims, token_dims, kernel_size=(3, 3), stride=PCMStride[2], padding=(1, 1),
                          groups=group),  # the 1st convolution
                nn.SiLU(inplace=True))
        ...
        self.num_patches = (img_size // downsample_ratios) * (
                    img_size // downsample_ratios)  # there are 3 sfot split, stride are 4,2,2 seperately
